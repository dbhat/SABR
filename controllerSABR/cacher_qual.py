...
serv_low_list=[<cache1_ip>, <cache2_ip>]
serv_high_list=[<cache3_ip>, <cache4_ip>]
...
low_qual_list = [89283, 262537, 791182]
high_qual_list = [2484135, 4219897]
def cache_miss(f_path, mpd_qual):
    ...
    for ip in server_list:
        ...
        if(table.find_one({"urn": f_path})) is not None:
            cache_insert=False
            continue
        if mpd_qual in low_qual_list:
            if ip in serv_low_list:
                 insert_qual=True
                 cache_insert=True
        elif mpd_qual in high_qual_list:
            if ip in serv_high_list:
                insert_qual=True
                cache_insert=True
        if cache_insert==True and insert_qual == True:
            try:
                client=pymongo.MongoClient(ip)
                print( "Connected successfully again!!!")
            except pymongo.errors.ConnectionFailure, e:
            ...
            if(table.find_one({"urn": f_path})) is None:
                ...
                print "Inserting URN: %s \n New cache Size %d in cache %s"%(f_path,estimated_cache_size,str(ip))
...
if __name__ == "__main__":
    ...
    while cursor.alive:
        ...
        try:
            ...
            cache_miss(str(doc["urn"]), int(doc['qual_no']))
